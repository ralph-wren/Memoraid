...
const autoFillContent = async () => {
  ...
  try {
    ...
    if (pageState === 'login') {
      ...
      // 【修复2026-04-01】检查是否已经在监听登录状态
      // 避免重复启动定时器导致无限循环
      const isMonitoring = sessionStorage.getItem('memoraid_login_monitoring');
      if (isMonitoring) {
        console.log('[Memoraid] 已经在监听登录状态，跳过重复处理');
        return;
      }
      ...
      // 【新增2026-04-01】标记正在监听登录状态
      sessionStorage.setItem('memoraid_login_monitoring', 'true');
      
      // 【新增2026-04-01】记录初始的 token 值
      ...
      // 不清除 pending 数据，等用户登录后由 Background 重新打开编辑器
      return;
    }
